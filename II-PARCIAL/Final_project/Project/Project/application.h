...
private:
    ...
    static inline void update_word();
};
...
inline void Application::administrator_menu()
{
    ...
    if (Utils::Login::login(user, password))
    {
        std::vector<MenuOption> menu_item{
            ...
            {6, "MODIFICAR UNA PALABRA", nullptr},
            {7, "CREAR ADMINISTRADOR", new_admin},
            {8, "MODIFICAR USUARIO", nullptr},
            {9, "CAMBIAR CONTRASEÑA", nullptr},
            {10, "REGRESAR", main_menu},
            {11, "SALIR", exit}};
...
inline void Application::init_tree(AvlTree &tree, int state, int type)
{
    ...
    {
        Utils::File::save_tree(tree, "dic2.txt");
    }
}
...
inline void Application::update_word()
{
    std::cout << "Ingrese la palabra a actualizar." << std::endl;
    std::string word = Utils::Validation::read_string("Palabra: ", Utils::InputType::WORD);
    AvlTree tree;
    Application::init_tree(tree, ApplicationState::RUNNING, Language::ENGLISH);
    while (true)
    {
        if (tree.has(word))
        {
            std::cout << "Ingrese la palabra nueva." << std::endl;
            std::string spanish_word = Utils::Validation::read_string("Español: ", Utils::InputType::WORD);
            std::string english_word = Utils::Validation::read_string("Ingles: ", Utils::InputType::WORD);
            tree.modify(spanish_word, english_word);
        }

        std::cout << "Desea actualizar otra palabra[s/n]: ";
        char option = 0;
        std::cin >> option;
        std::cin.ignore();
        if (option == 's' || option == 'S')
